...
import java.awt.*;
public class SearchRoomForCheckIn extends JFrame implements ActionListener {
	JButton next,cancle, search ;
	JComboBox<String> roomType;
	JCheckBox available;
	JTable table;
	public SearchRoomForCheckIn() {
		...
		available.setBounds(300, 70, 200, 25);
		add(available);
		...
		next.setFont(new Font("gisha", Font.PLAIN, 15));
		next.setBounds(570,420,100,30);
		next.addActionListener(this);
		add(next);
		
		search=new JButton("search");
		search.setFont(new Font("gisha", Font.PLAIN, 15));
		search.setBounds(570,70,100,30);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()==search) {
			Conn c=new Conn();
			try {
				String query1="SELECT rooms.roomnumber,rooms.availability,rooms.cleaning,rooms.roomtype,rooms.totalbalance, roomTypes.price FROM roomTypes JOIN rooms ON roomTypes.roomname = rooms.roomtype WHERE rooms.roomtype = '"+roomType.getSelectedItem() +"' ORDER BY rooms.roomnumber";
				String query2="SELECT rooms.roomnumber,rooms.availability,rooms.cleaning,rooms.roomtype,rooms.totalbalance, roomTypes.price FROM roomTypes JOIN rooms ON roomTypes.roomname = rooms.roomtype WHERE rooms.availability = 'Available' AND rooms.roomtype = '"+roomType.getSelectedItem() +"' ORDER BY rooms.roomnumber";
				ResultSet rs;
